...
//void nic_send_packet_main(THREADID tid);
...
VOID Instruction(INS ins) {
	...
	if (INS_IsXchg(ins) && INS_OperandReg(ins, 0) == REG_RBX && INS_OperandReg(ins, 1) == REG_RBX) {
        ...
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR) HandleNicMagicOp, IARG_THREAD_ID, IARG_REG_VALUE, REG_RBX, IARG_REG_VALUE, REG_RCX, IARG_END);
	}
