				...
				{                        
					...
					if (field == 0x16) {
						if (nicInfo->clean_recv) {
							...
							while (size) {
								reqSatisfiedCycle = max(l1d->clean(addr, dispatchCycle) + L1D_LAT, reqSatisfiedCycle);
								cRec.record(curCycle, dispatchCycle, reqSatisfiedCycle);
								addr += 64;
								size--;
							}
	...
	void OOOCore::NicMagicFunc(uint64_t core_id, OOOCore* core, ADDRINT val, ADDRINT field) {
		...
		switch (field) {
			...
			case 3: //get buf_size for lbuf. Is called before case 2 (but this was coded later)
				...
				info("num_cline = %d, val = %d, sizeof z_cacheline = %d",num_cline, val, sizeof(z_cacheline));
