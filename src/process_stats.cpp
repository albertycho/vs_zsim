...
void ProcessStats::updateCore(uint32_t cid, uint32_t p) {
	...
	//get around this temporarily?
    if(cCycles < lastCoreCycles[cid]){
		info("check for (cCycles >= lastCoreCycles[cid] && cInstrs >= lastCoreInstrs[cid]) failed, (%d) >= (%d)", cCycles, lastCoreCycles[cid]);

		cCycles = lastCoreCycles[cid];
    	cInstrs= lastCoreInstrs[cid];
	}
