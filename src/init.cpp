...
MemObject* BuildMemoryController(Config& config, uint32_t lineSize, uint32_t frequency, uint32_t domain, g_string& name) {
    ...
    if (type == "Simple") {
		...
		uint32_t var_8020= config.get<uint32_t>("sys.mem.var_8020", 0);

        mem = new SimpleMemory(latency, name, var_lat_radius, tail_var, var_8020);
    } else if (type == "MD1") {
...
static void InitSystem(Config& config) {
    ...
    zinfo->page_randomizer = gm_calloc<Page_Randomizer>();
    uint64_t gmPages = config.get<uint32_t>("sys.mem.capacityMB", (1 << 14))
                        *config.get<uint32_t>("sys.mem.controllers", 1)
                        *256;
    new (zinfo->page_randomizer) Page_Randomizer(gmPages, config.get<bool>("sim.pageRandomization", true));
...
void SimInit(const char* configFile, const char* outputDir, uint32_t shmid) {
    ...
    zinfo->perfect_bp = config.get<bool>("sim.perfect_bp", true);
