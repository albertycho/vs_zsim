...
# 2nd input: the base config file (assuming it's in the tests folder)
confile = sys.argv[2]
os.system('cp '+home+'/tests/'+confile+' .')

# 3rd input: some variable in the config file you want to use with multiple values, eg different L3 latencies
clwb_latency = sys.argv[3]

# 4th input: the zsim executable to use
zsim_exec = sys.argv[4]
...
while line:
	if 'command' in line:
		...
		tmp = line.split('=')[0]
		f2.write(tmp+'= True;\n')
	elif 'clwb_latency' in line:
		tmp = line.split('=')[0]
		f2.write(tmp+'= '+clwb_latency+';\n')
	else:
		f2.write(line)
	line = f1.readline()
f1.close()
