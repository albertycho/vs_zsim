...
int main() {
	...
	while(send_count<=32)
	{
		...
		//test_prints
		std::cout << "APP: recvd incoming msg.              recv_count:"<<std::dec << send_serviced << ", recv_buf_addr:" <<std::hex<< recv_completion.recv_buf_addr << ", recv_buf_val:" << *(uint32_t*)(recv_completion.recv_buf_addr) << std::endl;
